...
const v_getSignalIdentifiers = {
	Identifier: function (path)
	{
		...
		if (path.node.name[0] === '$' && !(path.node.name in this.names))
		{
			this.names[path.node.name] = true;

			path.node.name = path.node.name.substr(1);
...
const jsxTransformer = 
{
	...
	JSXElement (path)
	{
		...
		if (tagName[0] === tagName[0].toUpperCase())
		{
			...
			for (let i = 0; i < attributeList.length; i += 2)
			{
				...
				if (signalIds.length > 0)
					finalAttributes.push(t.objectProperty(attributeList[i],
						t.callExpression(this.context.id.expr, [t.arrayExpression(signalIds), t.arrowFunctionExpression(signalIds, exprPath.node)])
					));
				else
			...
			for (let i = 0; i < childList.length; i += 1)
			{
				...
				if (signalIds.length > 0 && !exprPath.node.isDynamicChild)
					finalChildren.push(t.callExpression(this.context.id.expr, [t.arrayExpression(signalIds), t.arrowFunctionExpression(signalIds, exprPath.node)]));
				else
		...
		else
		{
			...
			for (let i in dynamicAttributeList)
			{
				...
				let signalIds = getSignalIdentifiers(exprPath);
				if (signalIds.length > 0)
					dynamicAttributeList[i] = t.callExpression(this.context.id.expr, [t.arrayExpression(signalIds), t.arrowFunctionExpression(signalIds, exprPath.node)])
				else
			...
			for (let i in dynamicChildrenList)
			{
				...
				let signalIds = getSignalIdentifiers(exprPath);
				if (signalIds.length > 0 && !exprPath.node.isDynamicChild)
					dynamicChildrenList[i] = t.callExpression(this.context.id.expr, [t.arrayExpression(signalIds), t.arrowFunctionExpression(signalIds, exprPath.node)])
				else
	...
	CallExpression: function(path)
	{
		...
		let node = t.callExpression(this.context.id.expr, [t.arrayExpression(signalIds), t.arrowFunctionExpression(signalIds, path.node.arguments[0])]);
		node.isDynamic = true;
		path.replaceWith(node);
	}
};
