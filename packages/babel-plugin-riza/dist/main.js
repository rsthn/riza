...
const hasCallExpression = function (path) {
  ...
  if (path.node.type === 'JSXExpressionContainer') {
    if (path.node.expression.type === 'Identifier') {
      if (path.node.expression.name === 'children') return true;
    }
  }
  ...
  path.traverse({
    ImportSpecifier: function (path) {
      switch (path.node.imported.name) {
        case 'effect':
          this.effect = path.node.local.name;
          break;

        case 'replaceNode':
          this.replaceNode = path.node.local.name;
          break;
      }
    }
  }, i);
...
const flattenJsxElement = function (path) {
  ...
  for (let child of path.node.children) {
    if (t.isStringLiteral(child)) expr.push(child.value);else expr.push(child);
  }
...
export default function () {
  const visitor = {
    ...
    ImportDeclaration(path) {
      if (this.context.importedAll || path.node.source.value !== 'riza') return;
      ...
      for (let i in this.context.imported) this.context.importedAll &&= this.context.imported[i];
    },
    ...
    JSXElement(path) {
      ...
      if (path.scope.hasBinding(tagName)) {
        ...
        for (let i in path.node.openingElement.attributes) {
          ...
          if (attr.name.type === 'JSXIdentifier') attr.name = t.identifier(attr.name.name); // Style attribute can be set using a string, object or an expression.

          if (attr.name.name.toLowerCase() === 'style') {
            if (t.isStringLiteral(value)) {
              value = t.stringLiteral(value.split('\n').map(i => i.trim()).join(' ').trim());
            }
          } else {
        ...
        if (children.length) props.push(t.objectProperty(t.identifier('children'), t.arrayExpression(children)));
      ...
      _body.push(t.variableDeclaration('let', [t.variableDeclarator(_e, t.callExpression(t.memberExpression(t.identifier('document'), t.identifier('createElement')), [t.stringLiteral(tagName)]))])); // Process attributes.
      ...
      for (let i in path.node.openingElement.attributes) {
        ...
        let expr = null,
            value = attr.value; // Style attribute can be set using a string, object or an expression.
        ...
        if (attr.name.name.toLowerCase() === 'style') {
          if (t.isStringLiteral(value)) {
            ...
            switch (value.expression.type) {
              case 'CallExpression':
                expr = t.callExpression(this.context._effect, [t.arrowFunctionExpression([], t.blockStatement([t.expressionStatement(t.callExpression(t.memberExpression(t.identifier('Object'), t.identifier('assign')), [t.memberExpression(_e, _style), value.expression]))]))]);
                break;
              ...
              case 'ObjectExpression':
                ...
                for (let i in value.expression.properties) {
                  ...
                  switch (prop.value.type) {
                    default:
                      expr = t.callExpression(this.context._effect, [t.arrowFunctionExpression([], t.blockStatement([t.expressionStatement(t.assignmentExpression('=', t.memberExpression(t.memberExpression(_e, _style), prop.key), prop.value))]))]);
                ...
                if (obj.length) {
                  expr = t.callExpression(t.memberExpression(t.identifier('Object'), t.identifier('assign')), [t.memberExpression(_e, _style), t.objectExpression(obj)]);
                } else expr = false;
          ...
          if (value.type === 'JSXExpressionContainer') {
            expr = t.callExpression(this.context._effect, [t.arrowFunctionExpression([], t.blockStatement([t.expressionStatement(t.callExpression(t.memberExpression(_e, _setProperty), [t.stringLiteral(attr.name.name), value.expression]))]))]);
          }
        }

        if (expr === null) expr = t.callExpression(t.memberExpression(_e, _setProperty), [t.stringLiteral(attr.name.name), value]);
        if (expr) _body.push(t.expressionStatement(expr));
      }
      /* ** */
      ...
      if (inner.length > 0) {
        ...
        if (inner.length > 1 || strings > 0) {
          ...
          _body.push(t.expressionStatement(t.assignmentExpression('=', t.memberExpression(_e, t.identifier('innerHTML')), inner)));

          if (elems.length != 0) {
            _body.push(t.variableDeclaration('let', [t.variableDeclarator(_c, t.arrayExpression([]))]));
          }

          let _querySelector = t.identifier('querySelector');

          for (let i in elems) {
            _body.push(t.expressionStatement(t.assignmentExpression('=', t.memberExpression(_c, t.numericLiteral(Number(i)), true), t.callExpression(t.memberExpression(_e, _querySelector), [t.stringLiteral(`[data-__id='${i}']`)]))));
          }

          for (let i in elems) {
            _body.push(t.expressionStatement(t.callExpression(this.context._effect, [t.arrowFunctionExpression([], t.blockStatement([t.expressionStatement(t.assignmentExpression('=', t.memberExpression(_c, t.numericLiteral(Number(i)), true), t.callExpression(this.context._replaceNode, [t.memberExpression(_c, t.numericLiteral(Number(i)), true), elems[i]])))]))])));
          }
        } else {
          _body.push(t.expressionStatement(t.callExpression(this.context._effect, [t.arrowFunctionExpression([], t.blockStatement([t.expressionStatement(t.assignmentExpression('=', t.memberExpression(_e, t.identifier('innerHTML')), elems[0]))]))])));
        }
      }

      _body.push(t.returnStatement(_e));

      if (path.scope.parent) path.scope.push({
        kind: 'const',
        id: _id,
        init: fn
      });else this.context.decl.push(t.variableDeclaration('const', [t.variableDeclarator(_id, fn)]));
